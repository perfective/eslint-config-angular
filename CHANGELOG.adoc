...
** `@perfective/eslint-config-angular/template` and `@perfective/eslint-config-angular/community/template`
are merged into the `@perfective/eslint-config-angular` and `@perfective/eslint-config-angular/community`.
* Plugins `eslint-plugin-jest-dom` and `eslint-plugin-testing-library` are included,
but are now managed by the `@perfective/eslint-config` package.
* Updated plugins:
