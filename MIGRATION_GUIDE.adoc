...
export default perfectiveEslintConfigAngular([
    // ...Optional plugins configs...
    {
        rules: {
            ...
            '@angular-eslint/template/no-empty-control-flow': 'off',
...
+
If your project does not use ESM by default,
rename `eslint.config.js` into `eslint.config.mjs`.
+
** Update `eslint.config.js` to use ES `import` and the new flat config structure:
+
.Before
[source,javascript]
----
...
----
+
* *Remove `.eslintrc.html.js`*.
+
Angular templates are now linted by the same configuration.
+
...
(replaced by `@eslint-community/eslint-plugin-eslint-comments`)
*** `eslint-plugin-rxjs`
(replaced by `eslint-plugin-rxjsr-x`)
*** `eslint-plugin-rxjs-angular`
(replaced by `eslint-plugin-rxjs-angular-x`)
*** `eslint-plugin-sonarjs`
+
** Update customized rules (_if present_).
+
For example, change `rxjs-angular/prefer-takeuntil` to `rxjs-angular-x/prefer-takeuntil`.
+
* *Update imports and rules configuration*:
+
...
** `typescriptEslintNamingConvention` is moved from `@perfective/eslint-config/rules` to `@perfective/eslint-config/typescript-eslint`.
+
* *Re-import the following functions from `@perfective/eslint-config`* (if used):
+
...
const eslintConfig = perfectiveEslintConfigAngular([
    // ...Optional plugins configs...
    {
        rules: {
            // From @perfective/eslint-config v0.28-v0.34
